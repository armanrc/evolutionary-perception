...
to setup
  ...
  ask n-of 50 patches[set danger random(100) set pcolor brown set food 0]
  reset-ticks
end
...
to get-energy
  ...
  ask simple-strategy[ifelse change-env?[ifelse random(100) < mutation-rate [if any? neighbors with [food < (food-uper-limit / 2)]  [let maxpatch one-of neighbors with [food < (food-uper-limit / 2)]
    ...
    if not any? neighbors with [food < (food-uper-limit / 2)] [let neighbor-patch one-of neighbors let f [food] of neighbor-patch set energy energy + (f / simple-eating-rate-inverse)
      ask neighbor-patch[set food food - (f / simple-eating-rate-inverse)] ]][if any? neighbors with [food > (food-uper-limit / 2)]  [let maxpatch one-of neighbors with [food > (food-uper-limit / 2)]
      let f [food] of maxpatch  set energy energy + (f / simple-eating-rate-inverse)
      ask maxpatch[set food food - (f / simple-eating-rate-inverse)] ]
    if not any? neighbors with [food > (food-uper-limit / 2)] [let neighbor-patch one-of neighbors let f [food] of neighbor-patch set energy energy + (f / simple-eating-rate-inverse)
      ask neighbor-patch[set food food - (f / simple-eating-rate-inverse)] ]]][if any? neighbors with [food > (food-uper-limit / 2)]  [let maxpatch one-of neighbors with [food > (food-uper-limit / 2)]
